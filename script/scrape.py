...
def download():
        ...
        for i, image, in enumerate(image_data):
            for img in image['img']:
                if not os.path.isfile(
                    ...
                    if img.startswith("https://github.com"):
                        print(img)
                        ...
                        open(f"image/{image['user']}.{image['repo']}/"+img.split("?")[0].split(
                            "/")[-1], "wb").write(requests.get(img+"?raw=true").content)
    ...
    json.dump(image_data, open("images.json", "w"), indent=4)
...
def updateTask():
    ...
    for i, t in enumerate(task):
        ...
        *_, user, repo = task[i].split('/')
        if os.path.isdir(f"image/{user}.{repo}"):
            task[i] = f"- [x] {user}/{repo}"
        else:
            task[i] = f"- [ ] {user}/{repo}"

    open("task.md", "w").write("\n".join(task))


def scrapeleftOverCleanup():
    task = open("task.md").read().split("\n")
    for i, t in enumerate(task):
        if "[ ]" in task[i]:
            task[i] = t.split("] ")[-1]
            *_, user, repo = task[i].split('/')
            url = f"https://raw.githubusercontent.com/{user}/{repo}/master/README.rst"
            res = requests.get(url)
            if res.status_code == 200:
                open(f"data/{user}-{repo}.md", "w").write(res.text)
                print(f"{user}/{repo}")
